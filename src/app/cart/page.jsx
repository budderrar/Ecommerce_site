...
export default function cart() {
  let {getLoggedUserCart, updateCartProductQuantity, deleteCartItem, setitemNumber, itemNumber} = useContext(CartContext);
  ...
  async function deleteItem(productId){
    ...
    if(response.data.status == "success"){
      ...
      toast.error("prodcut did not delete")
    }
  }
...
<h2 className="text-center text-2xl text-emerald-600 font-bold capitalize my-4">total price: {CartDetails?.totalCartPrice}</h2>
...
<div className="relative overflow-x-auto shadow-md sm:rounded-lg">
  ...
  <Link href="/checkout"> <button className="btn my-3">Checkout</button></Link>
</div>


    
    </>: <h1 className='capitalize text-red-800 font-bold text-center text-3xl my-8 '>No Items To Shop</h1>}
